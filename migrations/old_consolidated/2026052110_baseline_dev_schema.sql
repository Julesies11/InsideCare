...
CREATE TABLE public.ic_staff_shifts (
    ...
    house_id uuid,
    shift_template character varying NOT NULL DEFAULT 'SIL'::character varying,
    notes text,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
...
DROP POLICY IF EXISTS "RBAC activity_log SELECT" ON public.ic_activity_log;
CREATE POLICY "RBAC activity_log SELECT" ON public.ic_activity_log FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('activity_log'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC branch_policies ALL" ON public.ic_branch_policies;
CREATE POLICY "RBAC branch_policies ALL" ON public.ic_branch_policies FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC branch_policies SELECT" ON public.ic_branch_policies;
CREATE POLICY "RBAC branch_policies SELECT" ON public.ic_branch_policies FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "RBAC branches ALL" ON public.ic_branches;
CREATE POLICY "RBAC branches ALL" ON public.ic_branches FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC branches SELECT" ON public.ic_branches;
CREATE POLICY "RBAC branches SELECT" ON public.ic_branches FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC checklist_item_master DELETE" ON public.ic_checklist_item_master;
...
DROP POLICY IF EXISTS "RBAC checklist_item_master SELECT" ON public.ic_checklist_item_master;
CREATE POLICY "RBAC checklist_item_master SELECT" ON public.ic_checklist_item_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC checklist_master DELETE" ON public.ic_checklist_master;
...
CREATE POLICY "RBAC checklist_master SELECT" ON public.ic_checklist_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC checklist_schedules ALL" ON public.ic_checklist_schedules;
CREATE POLICY "RBAC checklist_schedules ALL" ON public.ic_checklist_schedules FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC checklist_schedules SELECT" ON public.ic_checklist_schedules;
CREATE POLICY "RBAC checklist_schedules SELECT" ON public.ic_checklist_schedules FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR ic_jwt_has_house(house_id) OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC contact_types_master DELETE" ON public.ic_contact_types_master;
...
DROP POLICY IF EXISTS "RBAC contact_types_master SELECT" ON public.ic_contact_types_master;
CREATE POLICY "RBAC contact_types_master SELECT" ON public.ic_contact_types_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC departments ALL" ON public.ic_departments;
CREATE POLICY "RBAC departments ALL" ON public.ic_departments FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC departments SELECT" ON public.ic_departments;
CREATE POLICY "RBAC departments SELECT" ON public.ic_departments FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC employment_types_master DELETE" ON public.ic_employment_types_master;
...
CREATE POLICY "RBAC employment_types_master SELECT" ON public.ic_employment_types_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC error_logs ALL" ON public.ic_error_logs;
CREATE POLICY "RBAC error_logs ALL" ON public.ic_error_logs FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC error_logs INSERT" ON public.ic_error_logs;
...
DROP POLICY IF EXISTS "RBAC funding_sources_master DELETE" ON public.ic_funding_sources_master;
...
DROP POLICY IF EXISTS "RBAC funding_sources_master SELECT" ON public.ic_funding_sources_master;
CREATE POLICY "RBAC funding_sources_master SELECT" ON public.ic_funding_sources_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC funding_types_master DELETE" ON public.ic_funding_types_master;
...
DROP POLICY IF EXISTS "RBAC funding_types_master SELECT" ON public.ic_funding_types_master;
CREATE POLICY "RBAC funding_types_master SELECT" ON public.ic_funding_types_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_attachments ALL" ON public.ic_house_calendar_event_attachments;
CREATE POLICY "RBAC house_calendar_event_attachments ALL" ON public.ic_house_calendar_event_attachments FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_attachments.event_id) AND ic_jwt_has_house(hce.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_attachments SELECT" ON public.ic_house_calendar_event_attachments;
CREATE POLICY "RBAC house_calendar_event_attachments SELECT" ON public.ic_house_calendar_event_attachments FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_attachments.event_id) AND (ic_jwt_has_house(hce.house_id) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_participants ALL" ON public.ic_house_calendar_event_participants;
CREATE POLICY "RBAC house_calendar_event_participants ALL" ON public.ic_house_calendar_event_participants FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_participants.event_id) AND ic_jwt_has_house(hce.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_participants SELECT" ON public.ic_house_calendar_event_participants;
CREATE POLICY "RBAC house_calendar_event_participants SELECT" ON public.ic_house_calendar_event_participants FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_participants.event_id) AND (ic_jwt_has_house(hce.house_id) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_staff ALL" ON public.ic_house_calendar_event_staff;
CREATE POLICY "RBAC house_calendar_event_staff ALL" ON public.ic_house_calendar_event_staff FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_staff.event_id) AND ic_jwt_has_house(hce.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_staff SELECT" ON public.ic_house_calendar_event_staff;
CREATE POLICY "RBAC house_calendar_event_staff SELECT" ON public.ic_house_calendar_event_staff FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_calendar_events hce
  WHERE ((hce.id = ic_house_calendar_event_staff.event_id) AND (ic_jwt_has_house(hce.house_id) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_calendar_event_types_master DELETE" ON public.ic_house_calendar_event_types_master;
...
DROP POLICY IF EXISTS "RBAC house_calendar_event_types_master SELECT" ON public.ic_house_calendar_event_types_master;
CREATE POLICY "RBAC house_calendar_event_types_master SELECT" ON public.ic_house_calendar_event_types_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_calendar_events ALL" ON public.ic_house_calendar_events;
CREATE POLICY "RBAC house_calendar_events ALL" ON public.ic_house_calendar_events FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_calendar_events SELECT" ON public.ic_house_calendar_events;
CREATE POLICY "RBAC house_calendar_events SELECT" ON public.ic_house_calendar_events FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR ic_jwt_has_house(house_id) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_checklist_item_attachments ALL" ON public.ic_house_checklist_item_attachments;
CREATE POLICY "RBAC house_checklist_item_attachments ALL" ON public.ic_house_checklist_item_attachments FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE ((hcs.id = ic_house_checklist_item_attachments.submission_id) AND ic_jwt_has_house(hcs.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_checklist_item_attachments SELECT" ON public.ic_house_checklist_item_attachments;
CREATE POLICY "RBAC house_checklist_item_attachments SELECT" ON public.ic_house_checklist_item_attachments FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE ((hcs.id = ic_house_checklist_item_attachments.submission_id) AND (ic_jwt_has_house(hcs.house_id) OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_checklist_items ALL" ON public.ic_house_checklist_items;
CREATE POLICY "RBAC house_checklist_items ALL" ON public.ic_house_checklist_items FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_checklists hc
  WHERE ((hc.id = ic_house_checklist_items.checklist_id) AND ic_jwt_has_house(hc.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_checklist_items SELECT" ON public.ic_house_checklist_items;
CREATE POLICY "RBAC house_checklist_items SELECT" ON public.ic_house_checklist_items FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_house_checklists hc
  WHERE ((hc.id = ic_house_checklist_items.checklist_id) AND ic_jwt_has_house(hc.house_id))))));
DROP POLICY IF EXISTS "RBAC house_checklist_submission_items ALL" ON public.ic_house_checklist_submission_items;
CREATE POLICY "RBAC house_checklist_submission_items ALL" ON public.ic_house_checklist_submission_items FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE ((hcs.id = ic_house_checklist_submission_items.submission_id) AND ic_jwt_has_house(hcs.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_checklist_submission_items SELECT" ON public.ic_house_checklist_submission_items;
CREATE POLICY "RBAC house_checklist_submission_items SELECT" ON public.ic_house_checklist_submission_items FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE ((hcs.id = ic_house_checklist_submission_items.submission_id) AND (ic_jwt_has_house(hcs.house_id) OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_checklist_submissions DELETE" ON public.ic_house_checklist_submissions;
...
DROP POLICY IF EXISTS "RBAC house_checklist_submissions INSERT" ON public.ic_house_checklist_submissions;
CREATE POLICY "RBAC house_checklist_submissions INSERT" ON public.ic_house_checklist_submissions FOR INSERT TO authenticated WITH CHECK ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_checklist_submissions SELECT" ON public.ic_house_checklist_submissions;
CREATE POLICY "RBAC house_checklist_submissions SELECT" ON public.ic_house_checklist_submissions FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_checklist_submissions UPDATE" ON public.ic_house_checklist_submissions;
CREATE POLICY "RBAC house_checklist_submissions UPDATE" ON public.ic_house_checklist_submissions FOR UPDATE TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id)))) WITH CHECK ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_checklists DELETE" ON public.ic_house_checklists;
...
DROP POLICY IF EXISTS "RBAC house_checklists INSERT" ON public.ic_house_checklists;
CREATE POLICY "RBAC house_checklists INSERT" ON public.ic_house_checklists FOR INSERT TO authenticated WITH CHECK ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_checklists SELECT" ON public.ic_house_checklists;
CREATE POLICY "RBAC house_checklists SELECT" ON public.ic_house_checklists FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_checklists UPDATE" ON public.ic_house_checklists;
CREATE POLICY "RBAC house_checklists UPDATE" ON public.ic_house_checklists FOR UPDATE TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id)))) WITH CHECK ((ic_jwt_is_admin() OR (ic_jwt_get_perm('house_checklists'::text) = 'full'::text) OR ((ic_jwt_get_perm('house_checklists'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_comms ALL" ON public.ic_house_comms;
CREATE POLICY "RBAC house_comms ALL" ON public.ic_house_comms FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_comms SELECT" ON public.ic_house_comms;
...
CREATE POLICY "RBAC house_files ALL" ON public.ic_house_files FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_files SELECT" ON public.ic_house_files;
...
DROP POLICY IF EXISTS "RBAC house_form_assignments ALL" ON public.ic_house_form_assignments;
CREATE POLICY "RBAC house_form_assignments ALL" ON public.ic_house_form_assignments FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_forms hf
  WHERE ((hf.id = ic_house_form_assignments.form_id) AND ic_jwt_has_house(hf.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_form_assignments SELECT" ON public.ic_house_form_assignments;
CREATE POLICY "RBAC house_form_assignments SELECT" ON public.ic_house_form_assignments FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_forms hf
  WHERE ((hf.id = ic_house_form_assignments.form_id) AND ic_jwt_has_house(hf.house_id)))) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_form_submissions ALL" ON public.ic_house_form_submissions;
CREATE POLICY "RBAC house_form_submissions ALL" ON public.ic_house_form_submissions FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_house_forms hf
  WHERE ((hf.id = ic_house_form_submissions.form_id) AND ic_jwt_has_house(hf.house_id)))))));
DROP POLICY IF EXISTS "RBAC house_form_submissions SELECT" ON public.ic_house_form_submissions;
CREATE POLICY "RBAC house_form_submissions SELECT" ON public.ic_house_form_submissions FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_forms hf
  WHERE ((hf.id = ic_house_form_submissions.form_id) AND ic_jwt_has_house(hf.house_id)))) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_forms ALL" ON public.ic_house_forms;
CREATE POLICY "RBAC house_forms ALL" ON public.ic_house_forms FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC house_forms SELECT" ON public.ic_house_forms;
CREATE POLICY "RBAC house_forms SELECT" ON public.ic_house_forms FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR ic_jwt_has_house(house_id) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_resources ALL" ON public.ic_house_resources;
CREATE POLICY "RBAC house_resources ALL" ON public.ic_house_resources FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text) OR ((ic_jwt_get_perm('houses'::text) = 'context_read_write'::text) AND ic_jwt_has_house(house_id))));
...
CREATE POLICY "RBAC house_shift_templates SELECT" ON public.ic_house_shift_templates FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC house_staff_assignments ALL" ON public.ic_house_staff_assignments;
CREATE POLICY "RBAC house_staff_assignments ALL" ON public.ic_house_staff_assignments FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('employees'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC house_staff_assignments SELECT" ON public.ic_house_staff_assignments;
CREATE POLICY "RBAC house_staff_assignments SELECT" ON public.ic_house_staff_assignments FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (staff_id = ic_jwt_get_staff_id()) OR (ic_jwt_get_perm('employees'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ic_jwt_has_house(house_id) OR ic_jwt_manages_staff(staff_id)));
DROP POLICY IF EXISTS "RBAC house_types_master DELETE" ON public.ic_house_types_master;
...
CREATE POLICY "RBAC house_types_master SELECT" ON public.ic_house_types_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC houses ALL" ON public.ic_houses;
CREATE POLICY "RBAC houses ALL" ON public.ic_houses FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = 'full'::text)));
DROP POLICY IF EXISTS "RBAC houses SELECT" ON public.ic_houses;
CREATE POLICY "RBAC houses SELECT" ON public.ic_houses FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('houses'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND ic_jwt_has_house(id))));
DROP POLICY IF EXISTS "RBAC leave_requests ALL" ON public.ic_leave_requests;
CREATE POLICY "RBAC leave_requests ALL" ON public.ic_leave_requests FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR ((staff_id = ic_jwt_get_staff_id()) AND (status = 'pending'::text)) OR (ic_jwt_get_perm('leave_requests'::text) = 'full'::text) OR ((ic_jwt_get_perm('leave_requests'::text) = 'context_read_write'::text) AND ic_jwt_manages_staff(staff_id))));
DROP POLICY IF EXISTS "RBAC leave_requests SELECT" ON public.ic_leave_requests;
CREATE POLICY "RBAC leave_requests SELECT" ON public.ic_leave_requests FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (staff_id = ic_jwt_get_staff_id()) OR (ic_jwt_get_perm('leave_requests'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ic_jwt_manages_staff(staff_id)));
...
CREATE POLICY "RBAC leave_types SELECT" ON public.ic_leave_types FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
...
CREATE POLICY "RBAC medications_master SELECT" ON public.ic_medications_master FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC notifications ALL" ON public.ic_notifications;
...
CREATE POLICY "RBAC participant_contacts ALL" ON public.ic_participant_contacts FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_contacts.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_contacts SELECT" ON public.ic_participant_contacts;
CREATE POLICY "RBAC participant_contacts SELECT" ON public.ic_participant_contacts FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_contacts.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_documents ALL" ON public.ic_participant_documents;
CREATE POLICY "RBAC participant_documents ALL" ON public.ic_participant_documents FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
...
CREATE POLICY "RBAC participant_documents SELECT" ON public.ic_participant_documents FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_documents.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_forms ALL" ON public.ic_participant_forms;
CREATE POLICY "RBAC participant_forms ALL" ON public.ic_participant_forms FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_forms.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_forms SELECT" ON public.ic_participant_forms;
CREATE POLICY "RBAC participant_forms SELECT" ON public.ic_participant_forms FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_forms.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_funding ALL" ON public.ic_participant_funding;
CREATE POLICY "RBAC participant_funding ALL" ON public.ic_participant_funding FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_funding.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_funding SELECT" ON public.ic_participant_funding;
CREATE POLICY "RBAC participant_funding SELECT" ON public.ic_participant_funding FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_funding.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_goal_progress ALL" ON public.ic_participant_goal_progress;
CREATE POLICY "RBAC participant_goal_progress ALL" ON public.ic_participant_goal_progress FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM (ic_participant_goals pg
     JOIN ic_participants p ON ((p.id = pg.participant_id)))
  WHERE ((pg.id = ic_participant_goal_progress.goal_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_goals ALL" ON public.ic_participant_goals;
CREATE POLICY "RBAC participant_goals ALL" ON public.ic_participant_goals FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_goals.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_goals SELECT" ON public.ic_participant_goals;
CREATE POLICY "RBAC participant_goals SELECT" ON public.ic_participant_goals FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_goals.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_hygiene_routines ALL" ON public.ic_participant_hygiene_routines;
CREATE POLICY "RBAC participant_hygiene_routines ALL" ON public.ic_participant_hygiene_routines FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_hygiene_routines.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_hygiene_routines SELECT" ON public.ic_participant_hygiene_routines;
CREATE POLICY "RBAC participant_hygiene_routines SELECT" ON public.ic_participant_hygiene_routines FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_hygiene_routines.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_medications ALL" ON public.ic_participant_medications;
CREATE POLICY "RBAC participant_medications ALL" ON public.ic_participant_medications FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_medications.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_medications SELECT" ON public.ic_participant_medications;
CREATE POLICY "RBAC participant_medications SELECT" ON public.ic_participant_medications FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_medications.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_notes ALL" ON public.ic_participant_notes;
CREATE POLICY "RBAC participant_notes ALL" ON public.ic_participant_notes FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_notes.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_notes SELECT" ON public.ic_participant_notes;
CREATE POLICY "RBAC participant_notes SELECT" ON public.ic_participant_notes FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_notes.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participant_restrictive_practices ALL" ON public.ic_participant_restrictive_practices;
CREATE POLICY "RBAC participant_restrictive_practices ALL" ON public.ic_participant_restrictive_practices FOR ALL TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = 'full'::text) OR ((ic_jwt_get_perm('participants'::text) = 'context_read_write'::text) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_restrictive_practices.participant_id) AND ic_jwt_has_house(p.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_restrictive_practices SELECT" ON public.ic_participant_restrictive_practices;
CREATE POLICY "RBAC participant_restrictive_practices SELECT" ON public.ic_participant_restrictive_practices FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((p.id = ic_participant_restrictive_practices.participant_id) AND ic_jwt_has_house(p.house_id))))));
DROP POLICY IF EXISTS "RBAC participants ALL (Admin)" ON public.ic_participants;
...
CREATE POLICY "RBAC participants SELECT" ON public.ic_participants FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('participants'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND ic_jwt_has_house(house_id))));
DROP POLICY IF EXISTS "RBAC permission_mappings ALL (Admin)" ON public.ic_permission_mappings;
CREATE POLICY "RBAC permission_mappings ALL (Admin)" ON public.ic_permission_mappings FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC permission_mappings DELETE" ON public.ic_permission_mappings;
CREATE POLICY "RBAC permission_mappings DELETE" ON public.ic_permission_mappings FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC permission_mappings SELECT" ON public.ic_permission_mappings;
CREATE POLICY "RBAC permission_mappings SELECT" ON public.ic_permission_mappings FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC positions ALL (Admin)" ON public.ic_positions;
CREATE POLICY "RBAC positions ALL (Admin)" ON public.ic_positions FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC positions DELETE" ON public.ic_positions;
CREATE POLICY "RBAC positions DELETE" ON public.ic_positions FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC positions SELECT" ON public.ic_positions;
CREATE POLICY "RBAC positions SELECT" ON public.ic_positions FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC provider_participants ALL (Admin)" ON public.ic_provider_participants;
CREATE POLICY "RBAC provider_participants ALL (Admin)" ON public.ic_provider_participants FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC provider_participants DELETE" ON public.ic_provider_participants;
CREATE POLICY "RBAC provider_participants DELETE" ON public.ic_provider_participants FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC provider_participants SELECT" ON public.ic_provider_participants;
CREATE POLICY "RBAC provider_participants SELECT" ON public.ic_provider_participants FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC providers ALL (Admin)" ON public.ic_providers;
CREATE POLICY "RBAC providers ALL (Admin)" ON public.ic_providers FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC providers DELETE" ON public.ic_providers;
CREATE POLICY "RBAC providers DELETE" ON public.ic_providers FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC providers SELECT" ON public.ic_providers;
CREATE POLICY "RBAC providers SELECT" ON public.ic_providers FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC role_permissions ALL (Admin)" ON public.ic_role_permissions;
CREATE POLICY "RBAC role_permissions ALL (Admin)" ON public.ic_role_permissions FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC roles ALL (Admin)" ON public.ic_roles;
CREATE POLICY "RBAC roles ALL (Admin)" ON public.ic_roles FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC service_participants ALL (Admin)" ON public.ic_service_participants;
CREATE POLICY "RBAC service_participants ALL (Admin)" ON public.ic_service_participants FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC service_participants DELETE" ON public.ic_service_participants;
CREATE POLICY "RBAC service_participants DELETE" ON public.ic_service_participants FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC service_participants SELECT" ON public.ic_service_participants;
CREATE POLICY "RBAC service_participants SELECT" ON public.ic_service_participants FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC service_staff ALL (Admin)" ON public.ic_service_staff;
CREATE POLICY "RBAC service_staff ALL (Admin)" ON public.ic_service_staff FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC service_staff DELETE" ON public.ic_service_staff;
CREATE POLICY "RBAC service_staff DELETE" ON public.ic_service_staff FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC service_staff SELECT" ON public.ic_service_staff;
CREATE POLICY "RBAC service_staff SELECT" ON public.ic_service_staff FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC services ALL (Admin)" ON public.ic_services;
CREATE POLICY "RBAC services ALL (Admin)" ON public.ic_services FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC services DELETE" ON public.ic_services;
CREATE POLICY "RBAC services DELETE" ON public.ic_services FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC services SELECT" ON public.ic_services;
CREATE POLICY "RBAC services SELECT" ON public.ic_services FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC ic_shift_assigned_checklists ALL (Admin)" ON public.ic_shift_assigned_checklists;
CREATE POLICY "RBAC ic_shift_assigned_checklists ALL (Admin)" ON public.ic_shift_assigned_checklists FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_shift_notes ALL (Admin)" ON public.ic_shift_notes;
CREATE POLICY "RBAC ic_shift_notes ALL (Admin)" ON public.ic_shift_notes FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_shift_participants ALL (Admin)" ON public.ic_shift_participants;
CREATE POLICY "RBAC ic_shift_participants ALL (Admin)" ON public.ic_shift_participants FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC shift_template_checklists ALL (Admin)" ON public.ic_shift_template_checklists;
CREATE POLICY "RBAC shift_template_checklists ALL (Admin)" ON public.ic_shift_template_checklists FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC shift_template_checklists DELETE" ON public.ic_shift_template_checklists;
CREATE POLICY "RBAC shift_template_checklists DELETE" ON public.ic_shift_template_checklists FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC shift_template_checklists SELECT" ON public.ic_shift_template_checklists;
CREATE POLICY "RBAC shift_template_checklists SELECT" ON public.ic_shift_template_checklists FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC shift_template_default_checklists ALL (Admin)" ON public.ic_shift_template_default_checklists;
CREATE POLICY "RBAC shift_template_default_checklists ALL (Admin)" ON public.ic_shift_template_default_checklists FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC shift_template_default_checklists DELETE" ON public.ic_shift_template_default_checklists;
CREATE POLICY "RBAC shift_template_default_checklists DELETE" ON public.ic_shift_template_default_checklists FOR DELETE TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC shift_template_default_checklists SELECT" ON public.ic_shift_template_default_checklists;
CREATE POLICY "RBAC shift_template_default_checklists SELECT" ON public.ic_shift_template_default_checklists FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (ic_jwt_get_perm('master_lists'::text) = ANY (ARRAY['full'::text, 'read_only'::text]))));
DROP POLICY IF EXISTS "RBAC staff ALL (Admin)" ON public.ic_staff;
...
CREATE POLICY "RBAC staff SELECT" ON public.ic_staff FOR SELECT TO authenticated USING ((ic_jwt_is_admin() OR (id = ic_jwt_get_staff_id()) OR (ic_jwt_get_perm('employees'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ic_jwt_manages_staff(id) OR (EXISTS ( SELECT 1
   FROM ic_house_staff_assignments hsa
  WHERE ((hsa.staff_id = ic_staff.id) AND ic_jwt_has_house(hsa.house_id))))));
DROP POLICY IF EXISTS "RBAC ic_staff_compliance ALL (Admin)" ON public.ic_staff_compliance;
CREATE POLICY "RBAC ic_staff_compliance ALL (Admin)" ON public.ic_staff_compliance FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_staff_documents ALL (Admin)" ON public.ic_staff_documents;
CREATE POLICY "RBAC ic_staff_documents ALL (Admin)" ON public.ic_staff_documents FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_staff_shifts ALL (Admin)" ON public.ic_staff_shifts;
CREATE POLICY "RBAC ic_staff_shifts ALL (Admin)" ON public.ic_staff_shifts FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_staff_training ALL (Admin)" ON public.ic_staff_training;
CREATE POLICY "RBAC ic_staff_training ALL (Admin)" ON public.ic_staff_training FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC ic_timesheets ALL (Admin)" ON public.ic_timesheets;
CREATE POLICY "RBAC ic_timesheets ALL (Admin)" ON public.ic_timesheets FOR ALL TO authenticated USING (ic_jwt_is_admin());
DROP POLICY IF EXISTS "RBAC user_roles ALL (Admin)" ON public.ic_user_roles;
CREATE POLICY "RBAC user_roles ALL (Admin)" ON public.ic_user_roles FOR ALL TO authenticated USING (ic_jwt_is_admin());
...
CREATE POLICY "RBAC staff_documents INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_staff_documents'::text) AND (ic_jwt_is_admin() OR (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text) OR (ic_jwt_get_perm('employees'::text) = 'full'::text))));
DROP POLICY IF EXISTS "RBAC checklist_attachments INSERT" ON storage.objects;
CREATE POLICY "RBAC checklist_attachments INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_checklist_attachments'::text) AND (ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE (((hcs.id)::text = split_part(objects.name, '/'::text, 1)) AND ic_jwt_has_house(hcs.house_id) AND (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))))))));
DROP POLICY IF EXISTS "RBAC house_documents INSERT" ON storage.objects;
CREATE POLICY "RBAC house_documents INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_house_documents'::text) AND (ic_jwt_is_admin() OR (ic_jwt_has_house((split_part(name, '/'::text, 1))::uuid) AND (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))))));
DROP POLICY IF EXISTS "RBAC participant_photos INSERT" ON storage.objects;
CREATE POLICY "RBAC participant_photos INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_participant_photos'::text) AND (ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text])))));
DROP POLICY IF EXISTS "RBAC staff_photos INSERT" ON storage.objects;
CREATE POLICY "RBAC staff_photos INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_staff_photos'::text) AND (ic_jwt_is_admin() OR (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text) OR (ic_jwt_get_perm('employees'::text) = 'full'::text))));
DROP POLICY IF EXISTS "RBAC storage_objects INSERT" ON storage.objects;
CREATE POLICY "RBAC storage_objects INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK ((ic_jwt_is_admin() OR ((bucket_id = 'ic_checklist_attachments'::text) AND (ic_jwt_get_perm('house_checklists'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))) OR ((bucket_id = 'ic_participant_photos'::text) AND (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))) OR ((bucket_id = 'ic_participant_documents'::text) AND (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))) OR ((bucket_id = 'ic_staff_photos'::text) AND (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text)) OR ((bucket_id = 'ic_staff_documents'::text) AND (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text))));
DROP POLICY IF EXISTS "RBAC participant_documents INSERT" ON storage.objects;
CREATE POLICY "RBAC participant_documents INSERT" ON storage.objects FOR INSERT TO authenticated WITH CHECK (((bucket_id = 'ic_participant_documents'::text) AND (ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE (((p.id)::text = split_part(p.participant_name, '/'::text, 1)) AND ic_jwt_has_house(p.house_id) AND (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'context_read_write'::text]))))))));
DROP POLICY IF EXISTS "RBAC staff_documents SELECT" ON storage.objects;
CREATE POLICY "RBAC staff_documents SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_staff_documents'::text) AND (ic_jwt_is_admin() OR (ic_jwt_get_perm('employees'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text) OR ((ic_jwt_get_perm('employees'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND (EXISTS ( SELECT 1
   FROM ic_staff s
  WHERE (((s.id)::text = split_part(s.staff_name, '/'::text, 1)) AND ic_jwt_manages_staff(s.id))))))));
DROP POLICY IF EXISTS "RBAC branch_documents SELECT" ON storage.objects;
CREATE POLICY "RBAC branch_documents SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_branch_documents'::text) AND (ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_houses h
  WHERE (((h.branch_id)::text = split_part(h.house_name, '/'::text, 1)) AND ic_jwt_has_house(h.id)))))));
DROP POLICY IF EXISTS "RBAC staff_photos SELECT" ON storage.objects;
CREATE POLICY "RBAC staff_photos SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_staff_photos'::text) AND (ic_jwt_is_admin() OR (ic_jwt_get_perm('employees'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR (split_part(name, '/'::text, 1) = (ic_jwt_get_staff_id())::text) OR ((ic_jwt_get_perm('employees'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND (EXISTS ( SELECT 1
   FROM (ic_house_staff_assignments hsa
     JOIN ic_staff s ON ((s.id = hsa.staff_id)))
  WHERE ((((hsa.staff_id)::text = split_part(s.staff_name, '/'::text, 1)) OR (s.photo_url ~~* ('%'::text || s.staff_name))) AND ic_jwt_has_house(hsa.house_id))))))));
DROP POLICY IF EXISTS "RBAC house_documents SELECT" ON storage.objects;
CREATE POLICY "RBAC house_documents SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_house_documents'::text) AND (ic_jwt_is_admin() OR ic_jwt_has_house((split_part(name, '/'::text, 1))::uuid) OR (ic_jwt_get_perm('houses'::text) = ANY (ARRAY['full'::text, 'read_only'::text])))));
DROP POLICY IF EXISTS "RBAC participant_documents SELECT" ON storage.objects;
CREATE POLICY "RBAC participant_documents SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_participant_documents'::text) AND (ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('participants'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE (((p.id)::text = split_part(p.participant_name, '/'::text, 1)) AND ic_jwt_has_house(p.house_id))))))));
DROP POLICY IF EXISTS "RBAC checklist_attachments SELECT" ON storage.objects;
CREATE POLICY "RBAC checklist_attachments SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_checklist_attachments'::text) AND (ic_jwt_is_admin() OR (EXISTS ( SELECT 1
   FROM ic_house_checklist_submissions hcs
  WHERE (((hcs.id)::text = split_part(objects.name, '/'::text, 1)) AND ic_jwt_has_house(hcs.house_id)))))));
DROP POLICY IF EXISTS "RBAC participant_photos SELECT" ON storage.objects;
CREATE POLICY "RBAC participant_photos SELECT" ON storage.objects FOR SELECT TO authenticated USING (((bucket_id = 'ic_participant_photos'::text) AND (ic_jwt_is_admin() OR (ic_jwt_get_perm('participants'::text) = ANY (ARRAY['full'::text, 'read_only'::text])) OR ((ic_jwt_get_perm('participants'::text) = ANY (ARRAY['context_read_write'::text, 'context_read_only'::text])) AND (EXISTS ( SELECT 1
   FROM ic_participants p
  WHERE ((((p.id)::text = split_part(p.participant_name, '/'::text, 1)) OR (p.photo_url ~~* ('%'::text || p.participant_name))) AND ic_jwt_has_house(p.house_id))))))));
...
DROP TRIGGER IF EXISTS ic_update_participant_documents_updated_at_2 ON public.ic_participant_forms;
CREATE TRIGGER ic_update_participant_documents_updated_at_2 BEFORE UPDATE ON public.ic_participant_forms FOR EACH ROW EXECUTE FUNCTION ic_update_updated_at_column();
...
DROP TRIGGER IF EXISTS ic_trigger_handle_new_role_permissions ON public.ic_roles;
CREATE TRIGGER ic_trigger_handle_new_role_permissions AFTER INSERT ON public.ic_roles FOR EACH ROW EXECUTE FUNCTION ic_handle_new_role_permissions();
DROP TRIGGER IF EXISTS ic_trigger_handle_role_deletion_sync ON public.ic_roles;
CREATE TRIGGER ic_trigger_handle_role_deletion_sync BEFORE DELETE ON public.ic_roles FOR EACH ROW EXECUTE FUNCTION ic_handle_role_deletion_sync();
...
DROP TRIGGER IF EXISTS ic_trigger_sync_staff_role_to_metadata ON public.ic_staff;
CREATE TRIGGER ic_trigger_sync_staff_role_to_metadata AFTER INSERT ON public.ic_staff FOR EACH ROW EXECUTE FUNCTION ic_sync_staff_role_to_metadata();
DROP TRIGGER IF EXISTS ic_trigger_sync_staff_role_to_metadata_update ON public.ic_staff;
CREATE TRIGGER ic_trigger_sync_staff_role_to_metadata_update AFTER UPDATE ON public.ic_staff FOR EACH ROW EXECUTE FUNCTION ic_sync_staff_role_to_metadata();
