    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (sortedTemplates.length === 0) {
          return <span className="text-xs italic text-muted-foreground">No templates configured</span>;
        }

        return (
          <div className="flex flex-wrap gap-1.5 py-1">
            {sortedTemplates.map((st: any) => {
              const theme = getPeriodTheme(st.shift_template_name, st.color_theme);
              return (
                <Badge 
                  key={st.id} 
                  variant="outline"
