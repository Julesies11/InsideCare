...
export const ComplianceAttachmentsList = React.memo(function ComplianceAttachmentsList({
  staffId,
  userName,
  item,
  canEdit,
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      onAddAttachment({ file_name: data.file_name, file_path: data.file_path });
  ...
  return (
    <div className="space-y-1.5 pt-2 border-t border-dashed border-gray-200 mt-4">
      <Label className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block mb-2">Attachments</Label>
        ...
        {item.verifiedDocuments && item.verifiedDocuments.length > 0 ? (
          <div className="grid grid-cols-1 gap-1.5">
            {item.verifiedDocuments.map((doc, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center justify-between h-8 px-2.5 bg-white border border-gray-200 rounded text-[11px] group transition-colors hover:border-primary/30">
                  <button 
                    type="button"
                  ...
                  {canEdit && (
                    <button 
                      type="button"
                      onClick={() => doc.file_path && onRemoveAttachment(doc.file_path)}
          ...
          <span className="text-[10px] text-slate-400 italic block mb-1">No files attached</span>
        )}
        ...
        {canEdit && (
          <div className="relative mt-1">
            ...
            <Button
              ...
              size="sm"
              disabled={isUploading}
              onClick={() => document.getElementById(`file-${item.requirementId}`)?.click()}
