...
export function SeizureTypeMasterDialog({
  open,
  onClose,
  canEdit = true,
}: SeizureTypeMasterDialogProps) {
  const { data: items = [], isLoading } = useSeizureTypesMaster();
  const { mutateAsync: addType } = useAddSeizureTypeMaster();
  const { mutateAsync: updateType } = useUpdateSeizureTypeMaster();
  ...
  const handleToggleStatus = async (item: SeizureTypeMaster) => {
    try {
