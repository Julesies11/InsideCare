            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 p-4 bg-muted/30 rounded-lg border border-border/50 mb-4 min-w-0">
              <div className="space-y-1 min-w-0">
                ...
                <p className="text-sm font-medium pt-1 break-words whitespace-normal">
                  {formData.start_date
                    ? format(parseISO(formData.start_date as string), 'PPP')
                    : 'N/A'}
                </p>
              </div>
              <div className="space-y-1 min-w-0">
                ...
                <p className="text-sm font-medium pt-1 break-words whitespace-normal">
                  {formData.shift_time
                    ? (formData.shift_time as string).substring(0, 5)
                    : 'N/A'}
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="risks_observed">
                  Risks observed during shift?
                </Label>
                <RadioGroup
                  value={
                    formData.risks_observed === true
                      ? 'yes'
                      : formData.risks_observed === false
                        ? 'no'
                        : ''
                  }
                  onValueChange={(val) =>
              ...
              {formData.risks_observed && (
                <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1">
                  <Label
                    htmlFor="risk_description"
                    className="text-xs text-muted-foreground"
                  >
                    Describe any risks or concerns observed this shift. Include
                    what happened, what may have caused it, and what actions
                    were taken.
                  </Label>
                  <Textarea
                    id="risk_description"
                    ...
                    onChange={(e) =>
                      onFormChange('risk_description', e.target.value)
                    }
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Describe how the participant presented across the shift (e.g.
                mood, engagement, behaviour, energy levels). Avoid vague terms
                like ‘good’ or ‘fine’.
              </p>
              <Textarea
                ...
                onChange={(e) =>
                  onFormChange('overall_presentation', e.target.value)
                }
