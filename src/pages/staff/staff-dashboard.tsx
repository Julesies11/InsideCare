...
export function StaffDashboard() {
  ...
  const currentShift = upcomingSchedule.find(
    (item: any) =>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-red-700 mt-0.5">
                        You have {missingShiftNotesCount} completed shift
                        {missingShiftNotesCount !== 1 ? 's' : ''} missing a
                        shift note.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-orange-700 mt-0.5">
                        You have {missingTimesheetsCount} completed shift
                        {missingTimesheetsCount !== 1 ? 's' : ''} missing a
                        timesheet.
                      </p>
                    </div>
                  </div>
          ...
          {currentShift && (
            <Card
              className={cn(
                ...
                !currentShift.checklist_stats?.all_done &&
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Started at {currentShift.start_time.slice(0, 5)} ·
                        Scheduled until {currentShift.end_time.slice(0, 5)}
                      </p>

                      {currentShift.checklist_stats?.total > 0 && (
                        <div
                          className="mt-3 max-w-md"
                          data-testid="shift-checklist-progress"
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                              Shift Routines:{' '}
                              {currentShift.checklist_stats.completed} /{' '}
                              {currentShift.checklist_stats.total}
                              {!currentShift.checklist_stats.all_done && (
                          ...
                          <div className="h-1.5 w-full bg-gray-200 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(currentShift.checklist_stats.completed / currentShift.checklist_stats.total) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
          ...
          <Card className="border-0 sm:border">
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => navigate(ROUTES.MY_ROSTER)}
              >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-medium">
                            {format(new Date(item.start_date), 'EEE dd MMM')}
                          </p>
                          {(() => {
                            ...
                            return (
                              <Badge
                                variant="outline"
                                className={cn(
                                  'text-[9px] font-bold h-4 px-1 gap-1',
                                  theme.badge,
                                )}
                              >
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                          ...
                          {item.entry_type === 'event' && item.location
                            ? ` · ${item.location}`
                            : ''}
                        </div>
                ...
                {(pendingTimesheets.length > 0 ||
                  ...
                  >
                    {(() => {
                      ...
                      const pending = pendingTimesheets.filter(
                        (t: any) => t.status === 'pending',
                      ).length;

                      if (drafts > 0 && pending > 0)
                        return `${drafts} action, ${pending} pending`;
          ...
          <Card className="lg:col-span-2 border-0 sm:border">
            ...
            <CardContent>
              <div className="flex flex-wrap gap-3">
                <Button
                  variant="outline"
                  onClick={() => navigate(`${ROUTES.MY_LEAVE}/new`)}
                >
