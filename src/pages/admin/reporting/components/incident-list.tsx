  ...
  const { data: counts } = useQuery({
    ...
    queryFn: async () => {
      const getCountQuery = (status?: string) => {
        ...
        if (debouncedSearch) {
          q = q.or(
            `summary.ilike.%${debouncedSearch}%,details.ilike.%${debouncedSearch}%`,
          );
        }
        return q;
      };

      const [allRes, newRes, actionedRes, referredRes, closedRes] =
        await Promise.all([
          getCountQuery(),
          getCountQuery('New'),
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title="Incident ID" column={column} />
        ),
        size: 150,
        enablePinning: true,
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title="Incident Date" column={column} />
        ),
        size: 160,
        cell: ({ row }) => (
          <div className="flex flex-col text-left text-sm text-mono">
            <span className="font-normal text-gray-800 dark:text-gray-200">
              {format(new Date(row.original.incident_date), 'dd MMM yyyy')}
            </span>
            <span className="text-xs text-gray-400">
              {format(new Date(row.original.incident_date), 'HH:mm')}
            </span>
          </div>
        ),
      },
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-1">
            <span className="text-xs font-bold uppercase">
              {row.original.incident_type_info?.name || 'Unknown'}
            </span>
            ...
            <div className="relative">
              ...
              {searchQuery && (
                <Button
                  variant="ghost"
                  mode="icon"
                  className="absolute end-1 top-1/2 -translate-y-1/2 size-6"
