...
import { Plus, Edit, ShieldCheck, ArrowUpDown, ArrowUp, ArrowDown, Trash2, CheckCircle2, AlertTriangle } from 'lucide-react';
import { 
  useComplianceTypes, 
...
type IDDocInsert = Database['public']['Tables']['ic_id_document_types']['Insert'];

type SortField = 'compliance_name' | 'is_active' | 'attachment_applicable' | 'system_category';
...
export function ComplianceSettingsPage() {
  ...
  // Master List Hooks
  const { types = [], isLoading: loadingTypes, refetch: refetchTypes, error: typesError } = useComplianceTypes(true);
  const { mutateAsync: addType } = useAddComplianceType();
  const { mutateAsync: updateType } = useUpdateComplianceType();

  // ID Documents Hooks
  const { idDocumentTypes = [], isLoading: loadingDocs, refetch: refetchDocs, error: docsError } = useIDDocumentTypes(true);
  ...
  const handleToggleIDDocActive = async (doc: IDDocRow) => {
    ...
    try {
      ...
      toast.success(`ID document type ${newActive ? 'activated' : 'deactivated'} successfully`);
      refetchDocs();
    } catch (err: any) {
      toast.error('Failed to toggle active status: ' + err.message);
    }
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    if (sortField !== field) return <ArrowUpDown className="size-3.5 ms-2 text-muted-foreground/50" />;
    return sortDirection === 'asc' ? <ArrowUp className="size-3.5 ms-2 text-primary" /> : <ArrowDown className="size-3.5 ms-2 text-primary" />;
  };
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-muted-foreground">
              Manage the master list of mandatory and optional compliance checks.
            </p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 max-w-md">
            <TabsTrigger value="master-list">Compliance Master List</TabsTrigger>
              ...
              <CardHeader>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary" className="bg-slate-100 text-slate-500 font-bold text-[10px]">{sortedAndFilteredTypes.length}</Badge>
                    </div>
                  <Input
              ...
              <CardContent>
                {loadingTypes ? (
                  ...
                  <div className="py-10 text-center bg-red-50 rounded-lg border border-red-100 m-2">
                    ...
                    <p className="text-sm text-red-700 font-medium">Failed to load configurations</p>
                    <p className="text-xs text-red-500 mt-1">{(typesError as any).message}</p>
                  </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="w-[30%] text-center">Tracking Configuration</TableHead>
                          <TableHead
                          ...
                          <TableRow key={type.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <TableCell>
                              <div className="flex flex-wrap justify-center gap-1">
                                {type.expiry_date_applicable && (
                                  <Badge variant="outline" className="text-[10px] bg-blue-50 text-blue-700 border-blue-200">Expiry</Badge>
                                )}
                                {type.attachment_applicable && (
                                  <Badge variant="outline" className="text-[10px] bg-indigo-50 text-indigo-700 border-indigo-200">Attachment</Badge>
                                )}
                                {type.document_number_applicable && (
                                  <Badge variant="outline" className="text-[10px] bg-teal-50 text-teal-700 border-teal-200">Doc #</Badge>
                                )}
                                {type.comments_applicable && (
                                  <Badge variant="outline" className="text-[10px] bg-slate-50 text-slate-700 border-slate-200">Comments</Badge>
                                )}
                              </div>
                            </TableCell>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1">
                                <Button variant="ghost" size="sm" onClick={() => handleEditTypeClick(type)}>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="bg-slate-100 text-slate-500 font-bold text-[10px]">{sortedIDDocumentTypes.length}</Badge>
                </div>
                <CardDescription>
                  Define the types of identification documents and their respective point values.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loadingDocs ? (
                  ...
                  <div className="py-10 text-center bg-red-50 rounded-lg border border-red-100 m-2">
                    <AlertTriangle className="size-8 text-red-500 mx-auto mb-2" />
                    <p className="text-sm text-red-700 font-medium">Failed to load ID document types</p>
                    <p className="text-xs text-red-500 mt-1">{(docsError as any).message}</p>
                  </div>
                ) : idDocumentTypes.length === 0 ? (
                  <div className="py-10 text-center text-muted-foreground">
                    No ID document types configured. Click "Add ID Document Type" to create one.
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="w-[25%] text-center">Tracking Configuration</TableHead>
                      ...
                      <TableBody>
                        {sortedIDDocumentTypes.map((doc) => (
                          <TableRow key={doc.id} className="hover:bg-slate-50/50 transition-colors">
                            <TableCell>
                              <Badge variant="outline" className={doc.category === 'primary' ? 'bg-indigo-50 text-indigo-700' : 'bg-orange-50 text-orange-700'}>
                            ...
                            <TableCell className="text-center font-bold text-gray-900">{doc.points}</TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1">
                                <Button variant="ghost" size="sm" onClick={() => handleEditIDDocClick(doc)}>
                                  <Edit className="size-4" />
                                </Button>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className={doc.is_active ? 'text-destructive' : 'text-success'}
        ...
        <Dialog open={showTypeDialog} onOpenChange={setShowTypeDialog}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>{editingType ? 'Edit Compliance Type' : 'Add Compliance Type'}</DialogTitle>
              <DialogDescription>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTypeFormData({ ...typeFormData, compliance_name: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                ...
                <div className="flex flex-col gap-3 border-l ps-4">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1">Applicable Fields</Label>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="expiry-app" className="text-xs cursor-pointer">Expiry Date</Label>
                    <Switch
                      id="expiry-app"
                      checked={typeFormData.expiry_date_applicable}
                      onCheckedChange={(checked) => setTypeFormData({ ...typeFormData, expiry_date_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-num-app" className="text-xs cursor-pointer">Doc Number</Label>
                    <Switch
                      id="doc-num-app"
                      checked={typeFormData.document_number_applicable}
                      onCheckedChange={(checked) => setTypeFormData({ ...typeFormData, document_number_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="attach-app" className="text-xs cursor-pointer">Attachments</Label>
                    <Switch
                      id="attach-app"
                      checked={typeFormData.attachment_applicable}
                      onCheckedChange={(checked) => setTypeFormData({ ...typeFormData, attachment_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="comments-app" className="text-xs cursor-pointer">Comments</Label>
                    <Switch
                      id="comments-app"
                      checked={typeFormData.comments_applicable}
                      onCheckedChange={(checked) => setTypeFormData({ ...typeFormData, comments_applicable: checked })}
                    />
                  </div>
                </div>
              </div>
        ...
        <Dialog open={showIDDialog} onOpenChange={setShowIDDialog}>
          <DialogContent className="sm:max-w-[450px]">
            <DialogHeader>
              <DialogTitle>{editingIDDoc ? 'Edit ID Document Type' : 'Add ID Document Type'}</DialogTitle>
              <DialogDescription>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setIDFormData({ ...idFormData, name: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val: any) => setIDFormData({ ...idFormData, category: val })}
                  >
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setIDFormData({ ...idFormData, points: parseInt(e.target.value) || 0 })}
                  />
                </div>
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setIDFormData({ ...idFormData, placeholder: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-active" className="text-xs cursor-pointer">Active</Label>
                    <Switch
                      id="doc-active"
                      checked={idFormData.is_active}
                      onCheckedChange={(checked) => setIDFormData({ ...idFormData, is_active: checked })}
                    />
                  </div>
                </div>
                <div className="flex flex-col gap-3 border-l ps-4">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1">Applicable Fields</Label>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-expiry-app" className="text-xs cursor-pointer">Expiry Date</Label>
                    <Switch
                      id="doc-expiry-app"
                      checked={idFormData.expiry_date_applicable}
                      onCheckedChange={(checked) => setIDFormData({ ...idFormData, expiry_date_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-num-app" className="text-xs cursor-pointer">Doc Number</Label>
                    <Switch
                      ...
                      onCheckedChange={(checked) => setIDFormData({ ...idFormData, document_number_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-attach-app" className="text-xs cursor-pointer">Attachments</Label>
                    <Switch
                      id="doc-attach-app"
                      checked={idFormData.attachment_applicable}
                      onCheckedChange={(checked) => setIDFormData({ ...idFormData, attachment_applicable: checked })}
                    />
                  </div>
                  <div className="flex items-center justify-between space-x-2">
                    <Label htmlFor="doc-comments-app" className="text-xs cursor-pointer">Comments</Label>
                    <Switch
                      id="doc-comments-app"
                      checked={idFormData.comments_applicable}
                      onCheckedChange={(checked) => setIDFormData({ ...idFormData, comments_applicable: checked })}
                    />
                  </div>
                </div>
              </div>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowIDDialog(false)}>Cancel</Button>
