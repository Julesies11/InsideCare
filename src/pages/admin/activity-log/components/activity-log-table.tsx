...
function ChangeDiff({ changes }: { changes: any }) {
  ...
  if (changeEntries.length === 0) {
    return <p className="text-xs text-muted-foreground italic bg-slate-50 dark:bg-slate-900/50 p-3 rounded border border-dashed">No granular field changes recorded.</p>;
  }
    ...
    <div className="border rounded-lg overflow-hidden mt-2 bg-white dark:bg-slate-950">
      <table className="w-full text-xs">
        <thead className="bg-slate-50 dark:bg-slate-900 border-b">
          <tr>
            <th className="px-4 py-2.5 text-left font-bold text-gray-500 uppercase tracking-tight">Field</th>
            <th className="px-4 py-2.5 text-left font-bold text-gray-500 uppercase tracking-tight">Original</th>
            <th className="px-1 py-2.5 w-8"></th>
            <th className="px-4 py-2.5 text-left font-bold text-gray-500 uppercase tracking-tight">New Value</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-900">
          {changeEntries.map(([key, value]: [string, any]) => (
            <tr key={key} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
...
export function ActivityLogTable() {
  ...
  const [inspectingActivity, setInspectingActivity] = useState<ActivityLog | null>(null);
  ...
  const columns = useMemo<ColumnDef<ActivityLog>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const config = ACTIVITY_TYPE_BADGES[type] || { variant: 'secondary', icon: ShieldCheck };
        const Icon = config.icon;
        return (
          <Badge variant={config.variant} appearance="light" className="gap-1.5 capitalize py-0.5">
  ...
  return (
    <>
          ...
          <CardToolbar className="flex flex-col gap-4 p-5">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 w-full">
              <Tabs value={category} onValueChange={(v) => updateUrlParams({ category: v })}>
                <TabsList className="bg-slate-100 dark:bg-slate-900 border-none">
                  ...
                  <TabsTrigger value="logins_security">Logins & Security</TabsTrigger>
                </TabsList>
              </Tabs>
              ...
              <div className="relative w-full sm:w-64">
                ...
                />
                {localSearch && (
                  <Button mode="icon" variant="ghost" className="absolute end-1.5 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => setLocalSearch('')}>
            ...
            <div className="flex flex-wrap items-center gap-3 w-full border-t pt-4 border-slate-100 dark:border-slate-800">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider ps-1">Staff Member</label>
                <Select value={staff} onValueChange={(v) => updateUrlParams({ staff: v })}>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider ps-1">Module</label>
                <Select value={module} onValueChange={(v) => updateUrlParams({ module: v })}>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider ps-1">Date Range</label>
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      selected={dateRange}
                      onSelect={(r) => updateUrlParams({ from: r?.from?.toISOString() || null, to: r?.to?.toISOString() || null })}
                      numberOfMonths={2}
                    />
                  </PopoverContent>
                </Popover>
      ...
      <Dialog open={!!inspectingActivity} onOpenChange={(open) => !open && setInspectingActivity(null)}>
              ...
              <div className="space-y-6 pb-4">
                <div className="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 dark:border-slate-800">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm">{format(new Date(inspectingActivity.created_at), 'PPP p')}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">User</p>
                    <p className="text-sm font-medium">{inspectingActivity.user_name || 'System'}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Module</p>
                    <p className="text-sm capitalize">{inspectingActivity.entity_type.replace('_', ' ').replace('ic ', '')}</p>
                  </div>
                  <div className="space-y-1">
                ...
                {inspectingActivity.metadata && (
                  <div className="space-y-4">
                    ...
                    {inspectingActivity.activity_type === 'update' && inspectingActivity.metadata.changes ? (
                      <ChangeDiff changes={inspectingActivity.metadata.changes} />
                    ) : (
