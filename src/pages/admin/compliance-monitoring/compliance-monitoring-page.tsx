...
export function ComplianceMonitoringPage() {
  ...
  const columns: ColumnDef<ComplianceMonitoringItem>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataGridColumnHeader column={column} title="Requirement" />
      ),
      size: 250,
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-medium text-slate-700">
            {row.original.compliance_name}
          </span>
          {row.original.document_number && (
            <span className="text-[10px] text-muted-foreground">
              Ref: {row.original.document_number}
            </span>
          )}
        </div>
      ),
    },
    {
      ...
      header: ({ column }) => (
        <DataGridColumnHeader column={column} title="Expiry Date" />
      ),
      cell: ({ row }) => {
        ...
        switch (status) {
          ...
          case 'In Progress':
            return (
              <Badge
                variant="primary"
                className="gap-1.5 px-3 py-1 text-[11px] font-bold min-w-[100px] justify-center bg-blue-50 text-blue-700 border-blue-200"
              >
                In Progress
              </Badge>
            );
          case 'Missing':
            return (
              <Badge
                variant="secondary"
                className="gap-1.5 px-3 py-1 text-[11px] font-bold min-w-[100px] justify-center bg-slate-100 text-slate-600 border-slate-200"
              >
                Missing
              </Badge>
            );
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right pr-4">
          <Link
            to={`${ROUTES.STAFF_DETAIL}/${row.original.staff_id}?tab=compliance#req-${row.original.compliance_type_id}`}
          >
            <Button
              variant="ghost"
              size="sm"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-muted-foreground">
              Monitor individual staff compliance requirements and document
              expiries across the organization.
            </p>
          </div>
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-sm text-red-500 max-w-md mx-auto mt-2">
                  {(error as any).message ||
                    'An unexpected error occurred while fetching compliance records. Please check your network connection or permissions.'}
                </p>
                <Button
                  variant="outline"
                  size="sm"
