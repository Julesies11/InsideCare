...
import {
  ChecklistSubmission,
  useChecklistHistory,
} from '@/hooks/use-checklist-history';
...
export const HouseChecklistHistory = forwardRef<
  HouseChecklistHistoryRef,
  HouseChecklistHistoryProps
>((props, ref) => {
  ...
  } = useHouseChecklists(houseId || undefined);

  const [showExecutionDialog, setShowExecutionDialog] = useState(false);
  const [executingChecklist, setExecutingChecklist] = useState<any>(null);
  const [executingShiftId, setExecutingShiftId] = useState<string | undefined>(
    undefined,
  );
  ...
  const handleResume = async (submission: ChecklistSubmission) => {
    setIsResuming(true);
    const checklist = (houseChecklists || []).find(
      (c) => c.id === submission.checklist_id,
    );
    if (!checklist) {
      // If not in pre-loaded houseChecklists (maybe it's for another house), fetch it
      try {
        const clData = await checklistsApi.getHouseChecklist(
          submission.checklist_id,
        );

        if (!clData)
          throw new Error(
            'You do not have permission to view this checklist template',
          );
  ...
  const persistExecution = async (
    results: any,
    status: CHECKLIST_STATUS.in_progress | CHECKLIST_STATUS.completed,
  ) => {
    ...
    const submissionPayload = {
      ...
      completed_at:
        status === CHECKLIST_STATUS.completed ? new Date().toISOString() : null,
    };

    const submission = await checklistsApi.upsertSubmission(
      submissionPayload,
      submissionId,
    );
    ...
    const submissionItems = results.items.map((item: any) => {
      const originalItem = executingChecklist?.items?.find(
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title="Checklist" column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!houseInfo?.id)
            return (
              <span className="text-gray-600">{row.original.house_name}</span>
            );
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const isInProgress =
            row.original.status === CHECKLIST_STATUS.in_progress;
          return (
            <div className="text-center">
              {isInProgress ? (
                <Badge className="bg-primary/10 text-primary border-0 text-[10px] font-bold">
                  IN PROGRESS: {row.original.completed_item_count}/
                  {row.original.item_count}
                </Badge>
              ) : (
                <Badge
                  variant="outline"
                  className="bg-green-50 text-green-700 border-green-100 font-bold text-[10px]"
                >
                  COMPLETED: {row.original.completed_item_count}/
                  {row.original.item_count}
                </Badge>
              )}
            </div>
        ...
        <div className="flex items-center gap-3 p-4 bg-destructive/10 border border-destructive/20 rounded-xl animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-destructive/80">
              You have {incompleteYesterdayCount} checklist(s) with unfinished
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="secondary"
                      size="sm"
                      className="border border-gray-300 h-8 gap-1.5 font-bold"
                    >
          ...
          <div className="flex-1 px-6 pb-6 overflow-hidden">
            {executingChecklist && (
              <HouseChecklistExecution
                ...
                isReadOnly={
                  activeSubmission?.status === CHECKLIST_STATUS.completed
                }
                initialData={
                  activeSubmission
                    ? {
