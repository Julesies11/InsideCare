...
export function ScheduleChecklistsModal({
  open,
  onOpenChange,
  houseId,
  houseName,
  ...
  const { houseChecklists = [], loading: checklistsLoading } =
  ...
  const toggleChecklistInPattern = (
    weekIndex: number,
    dayIndex: number,
    checklistId: string,
  ) => {
  ...
  const handlePopulate = async () => {
    ...
    try {
      ...
      toast.error('Failed to schedule checklists', {
    ...
    return pattern.reduce((acc, week) => {
      return (
        ...
        Object.values(week).reduce(
          (weekAcc, dayChecklists) => weekAcc + dayChecklists.length,
          0,
        )
      );
    }, 0);
            ...
            <div className="flex items-center justify-between px-1">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Select which checklists to schedule for each day.
                </p>
              </div>
                            ...
                            <div className="p-2 space-y-1.5 flex-1 relative">
                              ...
                              {checklistsLoading ? (
                                ...
                                houseChecklists.map((checklist) => {
                                  const isSelected =
                                    !isBeforeStart &&
                                    weekPattern[day.id]?.includes(checklist.id);
