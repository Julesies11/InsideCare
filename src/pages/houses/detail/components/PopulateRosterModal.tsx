...
export function PopulateRosterModal({
  open,
  onOpenChange,
  houseId,
  houseName,
  onSuccess,
}: PopulateRosterModalProps) {
  ...
  useEffect(() => {
    if (
      open &&
      !hasInitializedParticipants.current &&
  ...
  useEffect(() => {
    ...
    setPattern((prev) => {
      ...
      while (newPattern.length < weeksToGenerate) {
        ...
        const lastWeek = newPattern[newPattern.length - 1] || {
          1: [],
          2: [],
          3: [],
          4: [],
  ...
  useEffect(() => {
    if (
      open &&
      !hasInitializedPattern.current &&
      shiftTemplates &&
      shiftTemplates.length > 0
    ) {
      const activeIds = shiftTemplates
  ...
  const copyMonToWeekdays = (weekIndex: number) => {
    ...
    toast.success(
      `Monday pattern copied to weekdays for Week ${weekIndex + 1}`,
    );
  };
  ...
  const handlePopulate = async () => {
    ...
    const calculatedEndDate = format(
      addDays(new Date(startDate), weeksToGenerate * 7 - 1),
      'yyyy-MM-dd',
    );
    ...
    try {
      const result = await materializePattern({
        ...
        shiftTemplates,
        defaults,
        participants: houseParticipants.filter((p) =>
    ...
    return pattern.reduce((acc, week) => {
      return (
        ...
        Object.values(week).reduce(
          (weekAcc, dayShifts) => weekAcc + dayShifts.length,
          0,
        )
      );
    }, 0);
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] flex flex-col p-0 overflow-hidden shadow-2xl border-none">
        <DialogHeader className="p-4 sm:p-5 border-b bg-white sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl sm:text-2xl font-black uppercase tracking-tight leading-none mb-1">
                Build Roster
              </DialogTitle>
              <DialogDescription className="text-xs sm:text-sm font-medium">
                Generate open confirmed shifts for {houseName} based on your
                shift templates.
              </DialogDescription>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pt-2 pb-6 space-y-5 bg-gray-50/30 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-2">
              <Label
                ...
                className="text-[10px] font-bold uppercase tracking-widest text-gray-500 ml-1 flex items-center gap-2"
              >
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between px-1">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Select which shift modes to include for each specific date.
                </p>
              </div>
            ...
            <div className="space-y-6">
              {pattern.map((weekPattern, weekIndex) => {
                const anchorMonday = startOfWeek(new Date(startDate), {
                  weekStartsOn: 1,
                });
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              isBeforeStart &&
                                'opacity-40 pointer-events-none grayscale bg-gray-50/50',
                            )}
                          >
                            ...
                            <div className="p-2 space-y-1.5 flex-1 relative">
                              ...
                              {shiftTemplates.length > 0 ? (
                                shiftTemplates.map((type) => {
                                  const isSelected =
                                    !isBeforeStart &&
                                    weekPattern[day.id]?.includes(type.id);
                                ...
                                <p className="text-[9px] text-muted-foreground text-center py-4 italic">
                                  No models
                                </p>
                              )}
                            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Select which participants should be assigned to these shifts.
              </p>
            </div>
            {houseParticipants.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {houseParticipants.map((p) => (
                  <div
                    key={p.id}
                    className="flex items-center space-x-3 bg-white border border-gray-200 p-3 rounded-xl hover:border-primary/30 transition-colors"
                  >
                    <Checkbox
                      id={`participant-${p.id}`}
                    ...
                    <label
                      ...
                      className="text-sm font-bold text-gray-700 leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer flex-1"
                    >
                      {p.name}
                    </label>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-5 flex gap-4">
              ...
              <div>
                <h5 className="text-xs font-black uppercase tracking-tight text-indigo-900">
                  Routine Checklists
                </h5>
                <p className="text-[10px] text-indigo-700 leading-relaxed mt-1">
                  Generated shifts will include:{' '}
                  {defaults.length > 0 ? (
                    <span className="font-bold">
                      {[
                        ...new Set(
                          defaults.map(
                            (d) =>
                              d.checklist?.house_checklist_name ||
                              d.checklist?.name,
                          ),
                        ),
                      ]
            ...
            <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 flex gap-4">
              ...
              <div>
                <h5 className="text-xs font-black uppercase tracking-tight text-emerald-900">
                  Participant Link
                </h5>
                <p className="text-[10px] text-emerald-700 leading-relaxed mt-1">
                  ...
                  will be linked to these shifts for documentation compliance.
                </p>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="p-6 bg-white border-t flex justify-between items-center sticky bottom-0 z-10">
          ...
          <div className="flex gap-3">
            <Button
              variant="outline"
              className="font-bold border-gray-300 h-11 px-6"
