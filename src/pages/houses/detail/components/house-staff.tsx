...
export function HouseStaff({
  ...
  const { data: houseStaffAssignments = [], isLoading: loading } =
    useHouseStaffAssignments(houseId);
  ...
  const handleEdit = (staffAssignment: {
    id?: string;
    tempId?: string;
    staff_id: string;
    is_primary?: boolean;
  ...
  const handleDelete = (staffAssignment: { id: string; tempId?: string }) => {
    ...
    if (
      confirm(
        'Mark this staff assignment for deletion? It will be removed when you click Save Changes.',
      )
    ) {
      const newPending = {
        ...pendingChanges,
  ...
  const handleCancelPendingAdd = (tempId: string) => {
    ...
    const newPending = {
      ...
      staff: {
        ...pendingChanges.staff,
        toAdd: pendingChanges.staff.toAdd.filter(
          (staff) => staff.tempId !== tempId,
        ),
      },
    };
    onPendingChangesChange(newPending);
  ...
  const handleCancelPendingUpdate = (id: string) => {
    ...
    const newPending = {
      ...
      staff: {
        ...pendingChanges.staff,
        toUpdate: pendingChanges.staff.toUpdate.filter(
          (staff) => staff.id !== id,
        ),
      },
    };
  ...
  const handleCancelPendingDelete = (id: string) => {
    ...
    const newPending = {
      ...
      staff: {
        ...pendingChanges.staff,
        toDelete: pendingChanges.staff.toDelete.filter(
          (staffId) => staffId !== id,
        ),
      },
    };
  ...
  const getStaffName = (staffAssignment: any) => {
    if (staffAssignment.staff?.staff_name)
      return staffAssignment.staff.staff_name;
  ...
  const getStaffPhoto = (staffAssignment: any) => {
    if (staffAssignment.staff?.photo_url)
      return staffAssignment.staff.photo_url;
    if (staffAssignment.staff_id) {
      const member = staff.find((s) => s.id === staffAssignment.staff_id);
        ...
        <CardHeader>
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-muted-foreground">
              Manage personnel assigned to this house
            </p>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {visibleStaffAssignments.map((staffAssignment) => {
                    ...
                    const isPendingUpdate = pendingChanges?.staff.toUpdate.some(
                      (s) => s.id === staffAssignment.id,
                    );
                    const isPendingDelete =
                      pendingChanges?.staff.toDelete.includes(
                        staffAssignment.id,
                      );

                    const isAssignmentActive =
                      !staffAssignment.end_date ||
                      new Date(staffAssignment.end_date) > new Date();
                    const isStaffActive =
                      staffAssignment.staff?.status === 'active';
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <SecureAvatar
                              ...
                              initials={getStaffName(staffAssignment)
                                .split(' ')
                                .map((w: string) => w[0])
                                .join('')
                                .toUpperCase()
                            ...
                            {isAssignmentActive &&
                              ...
                              !isPendingAdd && (
                                <div className="flex flex-col gap-1">
                                  <Badge
                                    variant="destructive"
                                    className="bg-orange-500 hover:bg-orange-600 border-none uppercase tracking-widest text-[9px] font-black"
                                  >
                                    Inactive
                                  </Badge>
                                  <span className="text-[10px] text-orange-600 font-bold italic uppercase">
                                    Employment:{' '}
                                    {staffAssignment.staff?.status || 'Unknown'}
                                  </span>
                                </div>
                              )}
                            {!isAssignmentActive && (
                          ...
                          <div className="flex justify-end gap-1">
                            {!isPendingDelete && (
                              <>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleEdit(staffAssignment)}
                                  disabled={!canAdd}
                                >
                            ...
                            {isPendingAdd && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleCancelPendingAdd(
                                    staffAssignment.tempId!,
                                  )
                                }
                              >
                                Remove
                            ...
                            {isPendingUpdate && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleCancelPendingUpdate(staffAssignment.id)
                                }
                              >
                                Undo
                              </Button>
                            ...
                            {isPendingDelete && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleCancelPendingDelete(staffAssignment.id)
                                }
                              >
                                Undo
                              </Button>
              ...
              {formData.staff_id && (
                <div className="flex items-center gap-3 bg-gray-50 px-2 py-1 rounded-lg border border-gray-100">
                  <div className="flex flex-col items-end">
                    <span className="text-[7px] font-black uppercase tracking-widest text-muted-foreground leading-none mb-0.5">
                      Status
                    </span>
                    <Badge
                      variant={
                        staff.find((s) => s.id === formData.staff_id)
                          ?.status === 'active'
                          ? 'success'
                          : 'secondary'
                      }
                    ...
                    >
                      {staff.find((s) => s.id === formData.staff_id)?.status ||
                        'Unknown'}
                    </Badge>
                  </div>
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-[9px] font-bold text-gray-700 leading-none">
                      {staff.find((s) => s.id === formData.staff_id)?.role
                        ?.role_name || 'None'}
                    </span>
                  </div>
                </div>
              ...
              {editingStaff ? (
                <div className="bg-white px-3 py-2 rounded-lg border border-gray-100 shadow-sm flex flex-col">
                  ...
                  <span className="text-sm font-black text-gray-900">
                    {staff.find((s) => s.id === formData.staff_id)?.name ||
                      'Unknown Staff'}
                  </span>
                </div>
              ) : (
