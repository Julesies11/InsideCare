...
import { ShiftNoteTask, useShiftNoteTasks } from '@/hooks/use-shift-notes';
...
const ShiftNotes = ({ participantId }: ShiftNotesProps) => {
    ...
    return tasks.filter((item) => {
      ...
      const matchesSearch =
        ...
        (item.participant_name &&
          item.participant_name.toLowerCase().includes(searchLower)) ||
        (item.staff_name &&
          item.staff_name.toLowerCase().includes(searchLower)) ||
        (item.house_name &&
          item.house_name.toLowerCase().includes(searchLower)) ||
        (item.shift_template &&
          item.shift_template.toLowerCase().includes(searchLower));
  ...
  const { houseCounts, staffCounts } = useMemo(() => {
    return tasks.reduce(
      (acc, item) => {
        if (item.house_id) {
          acc.houseCounts[item.house_id] =
            (acc.houseCounts[item.house_id] || 0) + 1;
        }
        if (item.staff_id) {
          acc.staffCounts[item.staff_id] =
            (acc.staffCounts[item.staff_id] || 0) + 1;
        }
        return acc;
      },
      {
        houseCounts: {} as Record<string, number>,
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className={cn('absolute inset-y-0 start-0 w-1', colorClass)} />
          );
        },
        size: 5,
        enableSorting: false,
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader title="Shift Note ID" column={column} />
        ),
        cell: ({ row }) => {
          const refId = row.original.note_reference_id;
          if (!refId)
            return (
              <div className="break-words whitespace-normal text-left text-sm text-gray-400 dark:text-gray-600">
                -
              </div>
            );
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!pId)
            return (
              <div className="break-words whitespace-normal text-left text-sm font-medium">
                {pName || '-'}
              </div>
            );
          ...
          return (
            <div className="break-words whitespace-normal text-left flex items-center min-w-0 w-full">
              <Link
                ...
                className="flex items-center gap-2 group/participant w-full min-w-0"
              >
                <SecureAvatar
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!sId)
            return (
              <div className="break-words whitespace-normal text-left text-sm text-gray-700 dark:text-gray-300">
                {sName || '-'}
              </div>
            );
          ...
          return (
            <div className="break-words whitespace-normal text-left flex items-center min-w-0 w-full">
              <Link
                ...
                className="flex items-center gap-2 group/staff w-full min-w-0"
              >
                <SecureAvatar
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!hId)
            return (
              <div className="break-words whitespace-normal text-left text-sm text-muted-foreground">
                {hName || '-'}
              </div>
            );
          ...
          return (
            <div className="break-words whitespace-normal text-left flex items-center min-w-0 w-full">
              <Link
                ...
                className="flex items-center gap-2 group/house w-full min-w-0"
              >
      ...
      <Card>
            ...
            <div className="flex items-center gap-1 bg-muted/30 p-1 rounded-lg border border-border/50">
              {STATUS_FILTERS.map((status) => (
                <Button
                  key={status}
                  variant={
                    activeStatusFilters.includes(status) ? 'primary' : 'ghost'
                  }
                  size="sm"
                  className={cn(
        ...
        {loading && (
          <div className="p-4 text-center">
            Loading shift documentation tasks...
          </div>
        )}
        {error && (
