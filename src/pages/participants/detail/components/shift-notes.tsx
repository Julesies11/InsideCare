...
export function ShiftNotes({
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useShiftNotesByParticipantId(participantId);
  ...
  const handleEdit = (note: ShiftNote & { tempId?: string }) => {
    if (note.tempId) {
      ...
      toast.info(
        'Please save the participant profile before editing this new note in the full editor.',
      );
      return;
    }
  ...
  const handleDelete = (note: any) => {
    ...
    if (
      confirm(
        'Mark this shift note for deletion? It will be removed when you click Save Changes.',
      )
    ) {
      const newPending = {
        ...pendingChanges,
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="py-10 text-center text-sm text-muted-foreground italic">
            No shift notes available for this participant
          </div>
        ) : (
          <div className="space-y-4">
            {allNotes.map((note: any) => {
              const isPendingDelete =
                !note.isPendingAdd &&
                pendingChanges?.toDelete.includes(note.id);
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isPendingDelete &&
                      'opacity-50 grayscale bg-red-50 border-red-200',
                  )}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                      ...
                      <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-900">
                        ...
                        {note.start_date
                          ? format(parseISO(note.start_date), 'dd MMM yyyy')
                          : '-'}
                        {shiftTimes && (
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {isPendingUpdate && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleUndoUpdate(note.id)}
                          disabled={!canEdit}
                        >
                          Undo
                        </Button>
                      ...
                      {isPendingDelete && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleUndoDelete(note.id)}
                          disabled={!canEdit}
                        >
                          Undo
                        </Button>
