...
export function getActiveOrganisationId(
  metadata?: UserAppMetadata | null,
): string {
  const DEFAULT_ORG_ID = '00000000-0000-0000-0000-000000000001';
  if (!metadata) return DEFAULT_ORG_ID;
  return (
    metadata.active_organisation_id ||
...
export function isEntityInActiveOrganisation(
  entityOrgId: string,
  activeOrgId: string,
): boolean {
...
export function simulateAuditTriggerOrganisationAssignment(
  ...
  const DEFAULT_PRIMARY_ORG = '00000000-0000-0000-0000-000000000001';

  if (TRACKED_TABLES.includes(tableName)) {
    if (
      !newOrgId ||
      (newOrgId === DEFAULT_PRIMARY_ORG && vOrgId !== DEFAULT_PRIMARY_ORG)
    ) {
      return vOrgId;
    }
  }
  return newOrgId || DEFAULT_PRIMARY_ORG;
}
...
describe('Multi-Tenant Context & JWT Claims Logic', () => {
  ...
  it('evaluates hybrid master list visibility rules correctly', () => {
    ...
    expect(isMasterItemVisibleForTenant(PRIMARY_ORG_ID, PRIMARY_ORG_ID)).toBe(
      true,
    );
    expect(
      isMasterItemVisibleForTenant(PRIMARY_ORG_ID, SECONDARY_ORG_ID),
    ).toBe(false);

    // Secondary Org item -> Visible ONLY to Secondary Org
    expect(
      isMasterItemVisibleForTenant(SECONDARY_ORG_ID, SECONDARY_ORG_ID),
    ).toBe(true);
    expect(
      isMasterItemVisibleForTenant(SECONDARY_ORG_ID, PRIMARY_ORG_ID),
    ).toBe(false);
  });

  it('enforces entity organisation isolation correctly', () => {
    expect(
      isEntityInActiveOrganisation(PRIMARY_ORG_ID, PRIMARY_ORG_ID),
    ).toBe(true);
    expect(
      isEntityInActiveOrganisation(PRIMARY_ORG_ID, SECONDARY_ORG_ID),
    ).toBe(false);
  });

  it('simulates ic_set_audit_columns trigger auto-population correctly', () => {
    // Missing org_id -> Auto-assign session org_id
    expect(
      simulateAuditTriggerOrganisationAssignment(
        null,
        SECONDARY_ORG_ID,
        'ic_houses',
      ),
    ).toBe(SECONDARY_ORG_ID);

    // Static default org_id when session is Secondary Org -> Auto-assign Secondary Org
    expect(
      simulateAuditTriggerOrganisationAssignment(
        PRIMARY_ORG_ID,
        SECONDARY_ORG_ID,
    ...
    // Explicit Secondary Org ID when session is Secondary Org -> Keep Secondary Org
    expect(
      simulateAuditTriggerOrganisationAssignment(
        SECONDARY_ORG_ID,
        SECONDARY_ORG_ID,
    ...
    // Non-tracked table (e.g. ic_leave_requests without org_id column) -> Untouched
    expect(
      simulateAuditTriggerOrganisationAssignment(
        null,
        SECONDARY_ORG_ID,
