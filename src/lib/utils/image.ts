...
export function validateImage(file: File): { error?: string } {
  ...
  if (file.size > MAX_SIZE) {
    return {
      error: `Image is too large (${(file.size / (1024 * 1024)).toFixed(1)}MB). Max 10MB allowed.`,
    };
  }
...
export async function compressImage(
  ...
  preset:
    ...
    | typeof COMPRESSION_PRESETS.AVATAR = 'AVATAR',
): Promise<File> {
  ...
  const options =
    typeof preset === 'string' ? COMPRESSION_PRESETS[preset] : preset;
