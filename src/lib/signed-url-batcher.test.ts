...
describe('SignedUrlBatcher', () => {
  ...
  it('should batch multiple requests into a single call', async () => {
    ...
    expect(supabase.storage.createSignedUrls).toHaveBeenCalledWith(['path1', 'path2'], 3600);
  });
  ...
  it('should chunk requests when exceeding CHUNK_SIZE (50)', async () => {
    ...
    expect(vi.mocked(supabase.storage.createSignedUrls).mock.calls[0][0]).toHaveLength(50);
    expect(vi.mocked(supabase.storage.createSignedUrls).mock.calls[1][0]).toHaveLength(25);
  }, 10000);
  ...
  it('should handle duplicate paths in the same batch', async () => {
    ...
    expect(supabase.storage.createSignedUrls).toHaveBeenCalledWith(['path1'], 3600);
  });
