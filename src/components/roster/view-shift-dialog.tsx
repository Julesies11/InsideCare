          ...
          {shift.notes && (
            <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-4 relative overflow-hidden group">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-blue-700">Instructions from Scheduler</span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Participants ({participantCount})</span>
              </div>
              <div className="space-y-2">
                {shift.participants && shift.participants.length > 0 ? (
                  ...
                  <p className="text-xs text-muted-foreground italic px-1">No participants assigned</p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {shift.assigned_checklists && shift.assigned_checklists.length > 0 ? (
                  shift.assigned_checklists.map(cl => (
                    <div key={cl.id} className="p-2.5 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
                      ...
                      {cl.items && cl.items.length > 0 && (
                        <div className="space-y-1 pl-1">
                          ...
                          {cl.items.length > 2 && (
                            <div className="text-[9px] text-muted-foreground/60 italic mt-0.5 ml-3 font-medium">
                              + {cl.items.length - 2} more items in checklist
                            </div>
                          )}
                        </div>
                      )}
