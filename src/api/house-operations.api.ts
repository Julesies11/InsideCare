...
export const houseOperationsApi = {
  ...
  calendar: {
    async list(params: { 
      ...
      const { houseId, startDate, endDate, view = CALENDAR_VIEWS.STANDARD, signal } = params;
      let query = supabase
    ...
    async upsert(event: Database['public']['Tables']['ic_house_calendar_events']['Insert'] | Database['public']['Tables']['ic_house_calendar_events']['Insert'][]) {
    ...
    async bulkDelete(params: { houseId: string, startDate: string, endDate: string, deleteChecklists?: boolean }) {
    ...
    async uploadAttachment(houseId: string, eventId: string, file: File, userId?: string) {
      ...
      const { data, error } = await supabase.from(TABLES.HOUSE_CALENDAR_EVENT_ATTACHMENTS).insert({
        event_id: eventId,
        file_name: file.name,
        file_path: filePath,
        file_size: file.size,
    ...
    async deleteAttachment(id: string, filePath: string) {
      await supabase.storage.from(STORAGE_BUCKETS.HOUSE_DOCUMENTS).remove([filePath]);
  ...
  forms: {
    ...
    async upsert(form: Database['public']['Tables']['ic_house_forms']['Insert'] | Database['public']['Tables']['ic_house_forms']['Insert'][]) {
  ...
  resources: {
    ...
    async upsert(resource: Database['public']['Tables']['ic_house_resources']['Insert'] | Database['public']['Tables']['ic_house_resources']['Insert'][]) {
  ...
  comms: {
    ...
    async create(comm: { house_id: string; content: string; entry_date: string }) {
      const { data, error } = await supabase
        .from(TABLES.HOUSE_COMMS)
        .insert([comm])
        .select(HOUSE_VIEWS.COMMS)
  ...
  async syncOperations(houseId: string, pending: HousePendingChanges) {
    ...
    if (pending?.staff?.toAdd?.length > 0) {
      ...
      const { error } = await supabase.from(TABLES.HOUSE_STAFF_ASSIGNMENTS).insert(staffInserts);
      if (error) errors.push(`Staff Add: ${error.message}`);
    }
    if (pending?.staff?.toUpdate?.length > 0) {
      for (const s of pending.staff.toUpdate) {
        const { error } = await supabase.from(TABLES.HOUSE_STAFF_ASSIGNMENTS).update({
    ...
    if (pending?.staff?.toDelete?.length > 0) {
      const { error } = await supabase.from(TABLES.HOUSE_STAFF_ASSIGNMENTS).delete().in('id', pending.staff.toDelete);
    ...
    if (pending?.participants?.toAdd?.length > 0) {
      for (const p of pending.participants.toAdd) {
        ...
        if (p.move_in_date !== undefined) updates.move_in_date = this.sanitizeValue(p.move_in_date);
        ...
        const { error } = await supabase.from(TABLES.PARTICIPANTS).update(updates).eq('id', p.participant_id);
        if (error) errors.push(`Participants Add ${p.participant_id}: ${error.message}`);
      }
    }
    if (pending?.participants?.toUpdate?.length > 0) {
      for (const p of pending.participants.toUpdate) {
        ...
        if (p.move_in_date !== undefined) updates.move_in_date = this.sanitizeValue(p.move_in_date);
        if (p.is_active !== undefined) updates.status = p.is_active ? 'active' : 'inactive';
        const { error } = await supabase.from(TABLES.PARTICIPANTS).update(updates).eq('id', p.id);
        if (error) errors.push(`Participant Update ${p.id}: ${error.message}`);
      }
    }
    if (pending?.participants?.toDelete?.length > 0) {
      const { error } = await supabase.from(TABLES.PARTICIPANTS).update({ house_id: null }).in('id', pending.participants.toDelete);
    ...
    if (pending?.calendarEvents?.toAdd?.length > 0) {
      for (const event of pending.calendarEvents.toAdd) {
        const { data: newEvent, error } = await supabase.from(TABLES.HOUSE_CALENDAR_EVENTS).insert({
          house_id: houseId,
          title: event.title,
        ...
        if (error) errors.push(`Event Add ${event.title}: ${error.message}`);
          ...
          if (event.participant_ids?.length) {
            await supabase.from(TABLES.HOUSE_CALENDAR_EVENT_PARTICIPANTS).insert(event.participant_ids.map(pId => ({ event_id: newEvent.id, participant_id: pId })));
          }
          if (event.assigned_staff_ids?.length) {
            await supabase.from(TABLES.HOUSE_CALENDAR_EVENT_STAFF).insert(event.assigned_staff_ids.map(sId => ({ event_id: newEvent.id, staff_id: sId })));
          }
        }
      }
    }
    if (pending?.calendarEvents?.toDelete?.length > 0) {
      const { error } = await supabase.from(TABLES.HOUSE_CALENDAR_EVENTS).delete().in('id', pending.calendarEvents.toDelete);
    ...
    if (pending?.checklists?.toAdd?.length > 0) {
      for (const cl of pending.checklists.toAdd) {
        const { data, error } = await supabase.from(TABLES.HOUSE_CHECKLISTS).insert({
          house_id: houseId,
    ...
    if (pending?.resources?.toAdd?.length > 0) {
      for (const res of pending.resources.toAdd) {
        ...
        if (res.file) {
          ...
          if (uploadError) {
            errors.push(`Resource File Upload ${res.title}: ${uploadError.message}`);
            continue;
          }
          file_url = filePath;
    ...
    if (pending?.resources?.toUpdate?.length > 0) {
      for (const res of pending.resources.toUpdate) {
        ...
        if (res.file) {
          ...
          if (uploadError) {
            errors.push(`Resource File Update ${res.title}: ${uploadError.message}`);
            continue;
          }
        ...
        const { error } = await supabase.from(TABLES.HOUSE_RESOURCES).update(updates).eq('id', res.id);
    ...
    if (pending?.resources?.toDelete?.length > 0) {
      for (const res of pending.resources.toDelete) {
        if (res.filePath) {
          await supabase.storage.from(STORAGE_BUCKETS.HOUSE_DOCUMENTS).remove([res.filePath]);
        }
        const { error } = await supabase.from(TABLES.HOUSE_RESOURCES).delete().eq('id', res.id);
