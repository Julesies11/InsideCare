...
describe('shiftNotesApi', () => {
  ...
  it('has a get method that fetches a single note by ID', async () => {
    ...
    const mockMaybeSingle = vi.fn().mockResolvedValue({ data: mockNote, error: null });
  ...
  it('upsert calls supabase with correct parameters and defaults to active status', async () => {
    ...
    expect(mockUpsert).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ onConflict: 'shift_id,staff_id,participant_id' })
    );
    expect(result).toEqual(mockNote);
  });

  it('create is an alias for upsert', async () => {
    const upsertSpy = vi.spyOn(shiftNotesApi, 'upsert').mockResolvedValue({ id: 'alias-id' } as any);
