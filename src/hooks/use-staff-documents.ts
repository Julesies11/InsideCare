  ...
  return useMutation({
    ...
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.STAFF_DOCUMENTS, data.staff_id],
      });
    },
  });
}
...
export function useDeleteStaffDocument() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.STAFF_DOCUMENTS, variables.staffId],
      });
    },
  });
}

export const getStaffFileUrl = async (
  filePath: string,
  downloadName?: string | boolean,
) => {
  return await staffDetailsApi.documents.getAttachmentSignedUrl(
    filePath,
    downloadName,
  );
};
