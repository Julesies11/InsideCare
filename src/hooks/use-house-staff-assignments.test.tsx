...
describe('useHouseStaffAssignments', () => {
  it('fetches house staff assignments when houseId is provided', async () => {
    server.use(
      http.get(
        `${SUPABASE_URL}/rest/v1/${TABLES.HOUSE_STAFF_ASSIGNMENTS}`,
        () => {
    ...
    const { result } = renderHook(() => useHouseStaffAssignments('house-1'), {
      wrapper,
    });
  ...
  it('does NOT fetch when houseId is undefined (default enabled guard)', () => {
    ...
    const { result } = renderHook(() => useHouseStaffAssignments(undefined), {
      wrapper,
    });
  ...
  it('returns empty assignments array when no data exists', async () => {
    ...
    const { result } = renderHook(
      () => useHouseStaffAssignments('house-empty'),
      { wrapper },
    );
  ...
  it('exposes loading state correctly during fetch', () => {
    ...
    const { result } = renderHook(
      () => useHouseStaffAssignments('house-loading'),
      { wrapper },
    );
