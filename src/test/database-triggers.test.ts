...
const createMockQueryBuilder = (responseData: any) => {
  const mock = {
    ...
    maybeSingle: vi.fn().mockImplementation(() => Promise.resolve(responseData)),
    then: vi.fn().mockImplementation((onFulfilled) => Promise.resolve(responseData).then(onFulfilled)),
  };
  return mock;
};
...
describe('Advanced Audit Trigger Expectations', () => {
  it('should verify the detailed description of a staff update', async () => {
    ...
    // Simulate the flow
    await supabase.from('ic_staff').update({ staff_name: 'Updated Audit Name' }).eq('id', staffId);
    ...
    expect(latestLog.metadata.changes.staff_name.new).toBe('Updated Audit Name');
  });
  ...
  it('should verify the parent context of a child record creation', async () => {
    ...
    const mockResponse = {
      data: [{
          ...
          entity_id: contactId,
          description: 'Added Contact "Emergency Contact" to Participant: Jane Doe',
          metadata: {
              new_data: {
    ...
    await supabase.from('ic_participant_contacts').insert({ name: 'Emergency Contact', participant_id: 'participant-uuid' });

    const { data: logs } = await supabase.from('ic_activity_log').select('*').eq('entity_id', contactId);
