...
describe('Modern Compliance DAL Verification', () => {
  ...
  describe('syncDetails (Defensive & No-Legacy Check)', () => {
    ...
    it('should block upserts if compliance_type_id is missing', async () => {
      const pending = {
        documents: { toAdd: [], toDelete: [] },
        training: { toAdd: [], toUpdate: [], toDelete: [] },
        staffCompliance: {
      ...
      await expect(
        staffDetailsApi.syncDetails('staff-123', pending),
      ).rejects.toThrow(/Compliance Add: Missing requirement ID/);
    });
  });
      ...
      (supabase.from as any).mockImplementation((table: string) => {
        if (table === TABLES.COMPLIANCE_TYPES_MASTER) {
          return {
            ...
            eq: vi
              .fn()
              .mockResolvedValue({ data: mockMasterTypes, error: null }),
          };
        }
        if (table === TABLES.STAFF_COMPLIANCE) {
          return {
            ...
            eq: vi
              .fn()
              .mockResolvedValue({ data: mockActualRecords, error: null }),
          };
        }
        return {};
      });
