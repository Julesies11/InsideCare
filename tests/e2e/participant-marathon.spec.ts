...
test.describe('Participant Detail Marathon CRUD', () => {
  ...
  test('Create and Fully Populate Participant Record', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Participant Details/i }).first(),
    ...
    const supportLevel = page.locator(
      'button:has-text("Select support level")',
    );
    ...
    await page
      ...
      .or(page.getByLabel(/Goal Description/i))
      .fill('Independent living skills and social inclusion');
    await page
    ...
    // 4. Behaviour Support
    await page
      .getByText(/Behaviour/i)
      .first()
      .click({ force: true });
    await page
      .locator('textarea#behaviour_of_concern')
      .or(page.getByLabel(/Behaviour of Concern/i))
      .fill('Occasional social anxiety in large groups.');
    await page
      .locator('input#specialist_name')
      .or(page.getByLabel(/Specialist Name/i))
      .fill('Dr. Behaviour');

    // 5. Support Needs (Communication)
    await page
      .getByText(/Support Needs/i)
      .first()
      .click({ force: true });
    await page
      .locator('textarea#routine')
      .or(page.getByLabel(/Routine/i))
      .fill('Morning walks, afternoon reading.');
    const commType = page.locator(
      'button:has-text("Select communication type")',
    );
    if (await commType.isVisible()) {
      await commType.click({ force: true });
      await page
        .getByRole('option', { name: 'Verbal', exact: true })
        .first()
        .click({ force: true });
    }
    await page
      .getByLabel(/Communication Type Notes/i)
      .fill('Prefers clear, direct instructions.');
    await page
      .locator('textarea#finance_support')
      .fill('Needs assistance with budget management.');
    ...
    await page
      ...
      .click();
    await page
      .locator('input#pharmacy_name')
      .or(page.getByLabel(/Pharmacy Name/i))
      .fill('Central Pharmacy');
    await page
      .locator('input#gp_name')
      .or(page.getByLabel(/GP Name/i))
      .fill('Dr. Smith');
    await page
      .locator('input#psychiatrist_name')
      .or(page.getByLabel(/Psychiatrist Name/i))
      .fill('Dr. Freud');

    // 8. Medications (Combobox + sub-entity CRUD)
    await page
      .getByText(/Medications/i)
      .first()
      .click();
    ...
    await page.getByRole('button', { name: /Add to Queue/i }).click();
    await expect(
      page.locator('#medications table tbody tr').first(),
    ).toBeVisible();
    ...
    await expect(
      page.locator('#documents').getByText('behavior_plan.pdf').first(),
    ).toBeVisible();
    ...
    await page
      ...
      .click();
    await page
      .locator('textarea#morning_routine')
      .fill('Wake up at 7am, brush teeth.');

    // 11. Restrictive Practices
    await page
      .getByText(/Practices/i)
      .first()
      .click();
    await page
      .locator('textarea#restrictive_practices')
      .fill('None currently authorized.');

    // 12. Notes
    await page.getByText(/Notes/i).first().click();
    await page
      .locator('textarea#general_notes')
      .fill('Participant is adjusting well to the new environment.');
    ...
    // Verify Persistence
    await expect(page.locator('input#participant_name')).toHaveValue(
      participantName,
    );
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.locator('input#specialist_name')).toHaveValue(
      'Dr. Behaviour',
    );
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.locator('textarea#finance_support')).toHaveValue(
      'Needs assistance with budget management.',
    );
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.locator('textarea#morning_routine')).toHaveValue(
      'Wake up at 7am, brush teeth.',
    );
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.locator('textarea#restrictive_practices')).toHaveValue(
      'None currently authorized.',
    );

    await page.getByText(/Notes/i).first().click();
    await expect(page.locator('textarea#general_notes')).toHaveValue(
      'Participant is adjusting well to the new environment.',
    );
    ...
    await expect(
      page.getByText(/Participant archived successfully/i),
    ).toBeVisible();
  });
});
