...
test.describe('House Detail Marathon CRUD', () => {
  ...
  test('Create and Fully Configure House Record', async ({ page }) => {
    ...
    await page
      ...
      .fill('456 Roster Way, Osborne Park WA');
    await page.locator('input#phone').fill('08 9222 3333');

    // 3. House Management (Section-by-section entry)
    await page
      .getByText(/Management/i)
      .first()
      .click();
    await page
      .getByPlaceholder(/Enter general house routines/i)
      .fill('Always check perimeter gates at 9 PM.');
    await page
      .getByPlaceholder(/Enter breakdown of individuals/i)
      .fill('3 adults with high support needs.');
    await page
      .getByPlaceholder(/social dynamics/i)
      .fill('Participants enjoy shared mealtimes.');
    await page
      .getByPlaceholder(/risk management/i)
      .fill('Wet floors in laundry during morning shifts.');
    await page
      .getByPlaceholder(/recent observations/i)
      .fill('Increased engagement during communal activities.');
    ...
    await page
      ...
      .fill('Kitchen Deep Clean');

    // Add a Task to the checklist
    await page.getByRole('button', { name: /Add Task/i }).click();
    await page
      .getByPlaceholder(/e.g. Check kitchen cleanliness/i)
      .fill('Wipe all countertops');
    ...
    await page
      .getByPlaceholder(/Resource title/i)
      .fill('Emergency Contacts List');
    ...
    await page
      .locator('textarea#comm_content')
      .fill('Initial setup log entry for house audit.');
    ...
    await page.getByRole('button', { name: /Save Changes/i }).click();
    await expect(page.getByText(/All changes saved successfully/i)).toBeVisible(
      { timeout: 20000 },
    );
    ...
    await expect(page.locator('textarea#address')).toHaveValue(
      '456 Roster Way, Osborne Park WA',
    );
    ...
    await page
      ...
      .click();
    await expect(
      page.getByPlaceholder(/Enter general house routines/i),
    ).toHaveValue('Always check perimeter gates at 9 PM.');
    await expect(page.getByPlaceholder(/recent observations/i)).toHaveValue(
      'Increased engagement during communal activities.',
    );
    ...
    const monCell = modal
      .locator('.bg-white.border.border-gray-100')
      .filter({ hasText: 'Mon' });
    ...
    await expect(page.getByText(/Roster populated successfully/i)).toBeVisible({
      timeout: 20000,
    });
    ...
    await expect(houseRow.getByText('SO').first()).toBeVisible({
      timeout: 10000,
    });
    ...
    await shiftDialog
      .getByPlaceholder(/Additional notes/i)
      .fill('Manual roster entry for verification.');
    ...
    await expect(
      houseRow.getByText('Manual roster entry for verification.'),
    ).toBeVisible();
