...
test.describe('Admin Workflows', () => {
  ...
  test('Admin can access Roster Board and see management tools', async ({
    page,
  }) => {
    ...
    // Check for management tools that are always visible (like the Staff or House filters)
    await expect(
      page
        .getByRole('combobox')
        .filter({ hasText: /All Houses|All Staff/i })
        .first(),
  ...
  test('Admin can view Activity Log', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Activity Log/i, level: 1 }),
    ).toBeVisible();
  ...
  test('Admin can access Role Management', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Roles/i, level: 1 }),
    ).toBeVisible();
