...
test.describe('DAL View Integrity Check', () => {
  ...
  const VIEWS_TO_CHECK = [
    {
      ...
      view: PARTICIPANT_VIEWS.DETAIL,
      name: 'Participant Detail',
    },
    {
      table: TABLES.PARTICIPANT_MEDICATIONS,
      view: PARTICIPANT_VIEWS.MEDICATIONS,
      name: 'Participant Medications',
    },
    {
      table: TABLES.PARTICIPANT_DOCUMENTS,
      view: PARTICIPANT_VIEWS.DOCUMENTS,
      name: 'Participant Documents',
    },
    { table: TABLES.STAFF, view: STAFF_VIEWS.DETAIL, name: 'Staff Detail' },
    {
      table: TABLES.STAFF_DOCUMENTS,
      view: STAFF_VIEWS.DOCUMENTS,
      name: 'Staff Documents',
    },
    {
      table: TABLES.HOUSES,
      view: HOUSE_VIEWS.STANDARD,
      name: 'House Standard',
    },
    {
      table: TABLES.HOUSE_RESOURCES,
      view: HOUSE_VIEWS.RESOURCES,
      name: 'House Resources',
    },
    { table: TABLES.HOUSE_COMMS, view: HOUSE_VIEWS.COMMS, name: 'House Comms' },
    {
      table: TABLES.STAFF_SHIFTS,
      view: ROSTER_VIEWS.SHIFT_DETAIL,
      name: 'Roster Shift Detail',
    },
  ];
  ...
  for (const item of VIEWS_TO_CHECK) {
    test(`Validate View: ${item.name}`, async ({ page }) => {
      ...
      expect(
        result.success,
        `View "${item.name}" failed: [${result.code}] ${result.message}\nHint: ${result.hint}`,
      ).toBe(true);
    });
  }
});
