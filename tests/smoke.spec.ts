...
async function discoverId(page, listPath, linkPattern) {
  try {
    ...
    console.warn(
      `Could not discover ID for ${linkPattern} at ${listPath}:`,
      e.message,
    );
  }
  return null;
...
for (const tab of PARTICIPANT_TABS) {
  staffTest(`Participant Detail tab ${tab} loads`, async ({ page }) => {
    ...
    if (isRealUuid) {
      ...
      console.log(
        `Skipping specific #id check for mock/invalid ID: ${targetId}`,
      );
    }
  });
}
...
publicTest(
  `Public user is redirected from protected page to signin`,
  async ({ page }) => {
...
staffTest(
  `Staff member is blocked from Admin Roster Board`,
  async ({ page }) => {
...
staffTest(
  `Staff member is blocked from Admin Activity Log`,
  async ({ page }) => {
